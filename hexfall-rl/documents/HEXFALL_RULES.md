...
**Bucket capacity:** Each bucket holds a fixed number of slices (parameter, default **24**) before filling. *Empirically validated May 16, 2026 against all 99 supported Paxie levels: every level's total slice count is exactly divisible by its bucket count at capacity 24, with zero exceptions. The previous value (25) was an unvalidated assumption from project setup; see `DECISIONS.md` May 16 entry.* When full, the bucket leaves the buffer and frees its slot.
...
- After thawing, the ice bucket behaves identically to a normal bucket: it is pickable when reachable, has the standard capacity (default 24), and contributes to slice-bucket parity exactly like any other bucket.
...
- A pin is **destroyed** when the cell **directly behind its origin ends a tick empty**. "Behind" means one step in the direction *opposite* the pin's facing direction. (Example: a pin at `(5, 3)` facing `Right` is destroyed when cell `(4, 3)` is empty; a pin at `(5, 3)` facing `Down` is destroyed when cell `(5, 2)` is empty. Coordinate semantics follow the level data convention where `y` increases downward.)
- Destruction is **quiescence-driven, not action-driven**: the destruction cell is checked at the end of each automatic-update tick. If a generator (located *elsewhere* in the grid) fires into the destruction cell on the same tick the player's pick emptied it, the cell is no longer empty at the time of the check and the pin survives. The pin's destruction cell must end an entire tick empty for the pin to be destroyed.
- When destroyed, **all cells of the ray become empty**, just as if each had been a bucket that was picked. Reachability is recomputed; previously unreachable regions on the far side of the ray may now become reachable. Cascading destruction is possible: if a destroyed pin's now-empty ray cells include the destruction cell of another pin, the second pin also destroys on the same tick.
- **Level-construction constraint:** the destruction cell of a pin must contain a pickable bucket (plain, ?, or ice) at level start — never a wall, never a generator, never another pin. This is a Paxie level-design invariant; the simulator may assume it on load and treat violations as invalid levels. A generator placed *elsewhere* that fires into the destruction cell *is* allowed; such configurations make the pin effectively permanent unless the generator's output queue runs dry and the destruction cell is subsequently emptied by a player pick that ends the tick uncontested.
...
- **Bucket capacity** — default 24 slices per bucket. Higher capacity = more slices buffered = more error tolerance.
...
- **May 16, 2026:** Default bucket capacity corrected in §4 from 25 to **24** slices, based on empirical validation against all 99 supported Paxie levels during issue #6 implementation. Every level's total slice count is exactly divisible by its bucket count at capacity 24, with zero exceptions. The previous value (25) was an unvalidated assumption from project setup; the corrected value reproduces real-Paxie-level parity exactly. See `DECISIONS.md` May 16 entry.