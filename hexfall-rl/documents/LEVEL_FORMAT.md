...
5. **Pin destruction cell.** For each pin, the cell directly opposite the origin (one step in the direction opposite the pin's facing) is verified to contain a pickable entity at level start — a `singleBlockCollector`, `woodBoxCollector`, or `iceCollector`. If it contains a wall, generator, or another pin, the level is malformed and the loader raises an error.
...
8. **Slice-bucket parity** (recommended warning, not hard error): the sum of all slice counts in `hexStackArea.stacks` should equal `bucket_count × bucket_capacity` (where `bucket_count` is the total of single + wood + ice collectors plus all queued generator outputs, and `bucket_capacity` is the simulator's per-bucket capacity, default 25 per `HEXFALL_RULES.md` §4). Real Paxie levels satisfy parity; procedurally generated levels under construction may not.
